...
const CommissionPayments = () => {
  ...
  const fetchPayments = async () => {
    try {
      ...
      const response = await api.get(`/admin/commission-payments?${params}`);
  ...
  const handlePaymentAction = async (paymentId, action) => {
    ...
    if (
      window.confirm(
        `Haqiqatan ham bu komissiya to'lovini ${actionText}ni xohlaysizmi?`
      )
    ) {
      try {
        await api.put(`/admin/commission-payments/${paymentId}/${action}`);

        if (action === "confirm") {
          alert(
            "Komissiya to'lovi tasdiqlandi! To'yxona egasi faollashtirildi."
          );
        } else {
          alert("Komissiya to'lovi rad etildi! To'yxona egasi nofaol qilindi.");
        }
          ...
          <div className="bg-white shadow overflow-hidden sm:rounded-md">
            ...
            {payments.length === 0 && (
              <div className="text-center py-8 text-gray-500">
                Hech qanday komissiya to'lovi topilmadi
              </div>
            )}
          </div>
        )}
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white p-4 rounded-lg max-w-2xl max-h-[90vh] overflow-auto">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-medium">Komissiya to'lov cheki</h3>
                <button
